...
.occup_db <- read.csv("https://zenodo.org/records/18630660/files/DB_V6_APRIL_2025_OCCUP_UTM_grids_V1.csv?download=1", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
#' #This provides Genera and Species Richness of the Subprovinces, National_Nature_Reserve s, and Natura_2000 areas  of France  and Spain
#' Data = CIPdb()
#' My_selection <- Select_CIPdb(Data, query = c("France", "Spain"))
#' My_areas  = c("Subprovince", "National_Nature_Reserve_Name", "Natura_2000_Name")
#' My_taxa_scope = c("Genus", "Species_with_Author")
#' title = "Spain_France_Species_Genus_Occupation"
#' My_tax_occup = Taxa_Occup_CIPdb(My_selection, My_taxa_scope , My_areas, output_name = title)
#' #This generates the occupation of the species of the genus Cotoneaster
#' My_selection <- Select_CIPdb(Data, query = "Cotoneaster")
#' My_areas  = c("Subprovince", "National_Nature_Reserve_Name", "Natura_2000_Name")
#' My_taxa_scope = c("Genus", "Species_with_Author")
#' title = "Cotoneaster_Species_Genus_Occupation"
#' My_tax_occup = Taxa_Occup_CIPdb(My_selection, My_taxa_scope , My_areas, output_name = title)
#' @export
...
.Protected_Areas <- st_read(system.file("ext", "Emerald_NatDA_2024_&_Natura_2000_CIP_2025.gpkg", package = "CelticInvasivePlantsdb"), driver = "GPKG", quiet = TRUE)
...
.mapa_UK_Crown_dependencies <- st_read(system.file("ext", "UK_Crown_dependencies_CIP.gpkg", package = "CelticInvasivePlantsdb"))
.mapa_Admin_units_II <- st_read(system.file("ext", "Admin_Units_II_CIP.gpkg", package = "CelticInvasivePlantsdb"))
.mapa_Admin_units_III <- st_read(system.file("ext", "Admin_Units_III_CIP.gpkg", package = "CelticInvasivePlantsdb"))
.Shp_31 <- st_read(system.file("ext", "Grids_CIP_2025.gpkg", package = "CelticInvasivePlantsdb"), layer = "Grids_Zone_31")
.Shp_30 <- st_read(system.file("ext", "Grids_CIP_2025.gpkg", package = "CelticInvasivePlantsdb"), layer = "Grids_Zone_30")
.Shp_29 <- st_read(system.file("ext", "Grids_CIP_2025.gpkg", package = "CelticInvasivePlantsdb"), layer = "Grids_Zone_29")

crs_objetivo <- st_crs(.Shp_30)
.Shp_31 <- st_transform(.Shp_31, crs_objetivo)

.Shp_29 <- st_transform(.Shp_29, crs_objetivo)
...
#' admin_Scope = c("Scotland", "Wales")
#' My_Area_type = "Constituent_Country_OR_Crown_Dependency"
#' title = "Scotland Wales Taxa Richness"
#' My_lines = c("UK_Crown_dependencies")
#' UTM_Rich_map(My_tax_rich, Admin_Scope = admin_Scope , Area_type = My_Area_type, Title = title, lines = My_lines, export_grids = TRUE)
#'#This provides the taxa richness of species in the French ATlantic subprovince
#' My_tax_rich = Taxa_Rich_CIPdb(Data, "Species_with_Author" , "UTM_grid")
#' admin_Scope = "French Atlantic"
#' title = "French Atlantic Taxa Richness"
#' My_reserves = c("Natura_2000", "National_Parks")
#' UTM_Rich_map(My_tax_rich, Admin_Scope = admin_Scope, Area_type = "Subprovince", Title = title, reserves = My_reserves,  export_grids = TRUE)
...
UTM_Rich_map<- function(data, Admin_Scope = "ALL", Area_type = "ALL", Title = "Taxa_Richness", export_grids = FALSE, lines = NULL, reserves = NULL) {
  ...
  map_plots <- lapply(richness_cols, function(richness_col) {
    ...
    # Crear el mapa
    map_plot_richness <- ggplot() +
      geom_sf(data = mapa_base, fill = "lightgray", color = "black") +
      geom_sf(data = do.call(rbind, map_data), aes(fill = !!sym(richness_col)), alpha = 0.6) +
...
#' @examples
#' #This provides a richness map of the France, Spain and Portugal at "Admin_units_II"
#' Data = CIPdb()
...
#' title = "Taxa_Richness_Continent_Admin_II"
#' Admin_Rich_Occup_map(My_tax_rich, Representation_type = "richness", Admin_Scope = "Admin_units_II", Title = title)
#'#This provides a Cortaderia selloana occupation  of the French, Spain and Portugal at "Admin_units_II"
#' My_selection <- Select_CIPdb(Data,query = c("France", "Spain", "Portugal"))
...
#' title = "Cortaderia_selloana_Occupation_Continent_Admin_II"
#' Admin_Rich_Occup_map(My_tax_occup, Representation_type = "occupation", Admin_Scope = "Admin_units_II", Title = title)
#' #This provides a species richness map of the French, Spain and Portugal at "Natura_2000_Name"
#' Data = CIPdb()
#' My_selection <- Select_CIPdb(Data, query = c("France", "Spain", "Portugal"))
#' My_taxa_scope = c("Family", "Genus", "Species_with_Author")
#' My_tax_rich = Taxa_Rich_CIPdb(My_selection, My_taxa_scope, "Natura_2000_Name")
#' title = "Taxa_Richness_Continent_Natura_2000_Name"
#' Admin_Rich_Occup_map(My_tax_rich, Representation_type = "richness", Admin_Scope = "Natura_2000_Name", Title = title)
#' @export
...
#'admin_query = c("Spain", "France")
#'title = "Acacia_Distribution"
#'My_lines = c("Admin_units_II")
#'My_reserves = c("Natura_2000", "National_Parks")
#'G = "Genus"
#'Tax_Distribution_map(Data, Taxa_Scope = G, "Acacia", Admin_query = admin_query, Admin_Scope = "Country", Title = title, export_grids = FALSE, lines = My_lines, reserves = My_reserves)
...
#' @description  This function generate Taxa Distribution maps (.png and .svg) with different administrative resolutions.
#' @param data argument must be the table obtained from CIPdb() or another selection of this (Select_CIPdb, WDPA_PID_select_CIPdb).
#' @param query argument allows to select the name or names of the taxa of interest. It must be either a character string or a vector of character strings specifying the term or terms of interest. This term must coincide with a value within the specified Scope_taxa (i.e. the value must be in the Scope_taxa column).
#' @param Taxa_Scope argument allows to specify the taxa category of the query. It must be be either a character string or a vector of character strings. It must be one of these: Phylum, Subphylum, Order, Family, Genus and Species_with_Author.
...
#' @param Admin_Scope argument allows to specify the Administrative category of the query. It must be be either a character string or a vector of character strings. It must be one of these: "Subprovince", "Country", "Admin_units_II", "Constituent_Country_OR_Crown_Dependency","Admin_units_III", "Natura_2000" and "National_Parks".
#' @param Title this allows the user to assign a name for the output folder.
...
#' #This provides distribution of Abutilon theophrasti and Phytolacca americana in the whole Celtic Fringe.
#' Data = CIPdb()
#' My_species = c("Abutilon theophrasti", "Phytolacca americana")
#' admin_query = "Natura_2000_Name"
#' admin_Scope = "Natura_2000_Name"
#' title = "Taxa_Distribution"
#'Tax_Distribution_Admin_map(Data, query = My_species, Admin_query = admin_query, Admin_Scope = admin_Scope, Title = title)
#'#This provides distribution of the genus Acacia in the Spain and France representing the reserves as well
#'admin_query = c("Spain", "France")
#'title = "Acacia_Distribution"
#'Tax_Distribution_Admin_map(Data, Taxa_Scope = "Genus", "Acacia", Admin_query = admin_query, Admin_Scope = "Country", Title = title)
#' @export
Tax_Distribution_Admin_map <- function(data, query, Taxa_Scope = "Taxa", Admin_query = "Country", Admin_Scope = "Country", Title = "Taxa_Distribution") {
  ...
  for (q in query) {
    # Filtrar los datos según el query
    data_filtered <- data[data[, Taxa_Scope] == q & Reduce(`|`, lapply(Admin_query, function(x) grepl(x, data[[Admin_Scope]], ignore.case = TRUE))), ]

    # Unir mapa con data
    mapa_filtrado <- mapa[Reduce(`|`, lapply(Admin_query, function(x) grepl(x, mapa[[Admin_Scope]], ignore.case = TRUE))), ]
    mapa_filtrado <- merge(mapa_filtrado, data_filtered, by = Admin_Scope, all.x = TRUE)
    ...
    map_data$Presente <- ifelse(!is.na(map_data[[Taxa_Scope]]), "Presente", "No Presente")
